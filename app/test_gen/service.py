...
def generate_test(
    ...
    # In case the rounded one is not the exact.
    if len(test_set) < math_total_questions:
        ...
        for i in range(number_generate_more):
            # randomly select for the rest of the slots.
        ...
        test_set = test_set[:57]
    num_list.append(len(test_set))
    num_list.append(number_generate_more)
    ...
    if len(test_set) < total_questions:
        ...
        test_set = test_set + addtional_list
    elif len(test_set) > total_questions:
        test_set = test_set[58:153]
    #TODO: insert the test_set into the database.
    supabase.auth.set_session(access_token, refresh_token)
    user_id = supabase.auth.get_user().user.id
    complete_problem_set = CompleteTestSet(
        name="New Test",
        is_full_test=True,
        user_id=user_id,
        set=test_set 
